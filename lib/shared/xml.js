  ...
  try {
    if (which === "xmldom") {
      ...
      const root = new DOMParser({
        ...
        onError: (level, msg /*, locator */) => {
          if (level === "warning") return; // ignore warnings
          throw new Error(msg);
        }
      }).parseFromString(xml, "application/xml").documentElement;

      return callback(null, root);
    } else {
      // libxml path
      const parser = require("libxml/lib/libxml");
      const root = exports.xmlParseError(
        parser.parseFromString(xml).documentElement
      );
      return callback(null, root);
    }
  } catch (ex) {
