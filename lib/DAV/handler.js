            ...
            if (internalMethods[method]) {
                ...
                self.dispatchEvent("unknownMethod", method, uri, function(stop) {
                    if (stop === true)
                        return;
                    // Unsupported method
                    self.handleError(stop || new Exc.NotImplemented("Unknown Method " + method));
                });
            }
        });
    };
            ...
            Xml.loadDOMDocument(data, self.server.options.parser, function(err, dom) {
                ...
                self.dispatchEvent("report", reportName, dom, function(stop) {
                    if (stop !== true) {
                        ...
                        return self.handleError(new Exc.ReportNotImplemented("This report is not supported"));
                    }
                });
            });
        });
    ...
    this.getHTTPPrefer = function() {
        ...
        if (prefer) {
            prefer.split(",").map(function(item) {
                ...
                // Parser supports https://tools.ietf.org/html/rfc7240
                var matches = parameter.match(/^([a-z0-9\-]+)(?:=([a-z0-9]+))?$/);

                if (matches !== null && matches.length > 2) {
                    switch(matches[1]) {
                        case "respond-async":
                            result["return-asynch"] = true;
                            break;
                        case "return":
                            switch(matches[2]) {
                                case "minimal":
                                    ...
                                    break;
                                case "representation":
                                    result["return-representation"] = true;
                                    break;
                            }
                            break;
                        case "handling":
