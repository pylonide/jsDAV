...
var jsCardDAV_Plugin = module.exports = jsDAV_Plugin.extend({
    ...
    validateVCard: function(data) {
        ...
        // This fix an issue when syncing with MacOS addressbook: the UID of the object
        // is not in vobj but in its first child
        if (!vobj.UID) {
            if (Array.isArray(vobj.children)) {
                vobj.children.forEach(function (child) {
                    if (child.name === 'UID') {
                        vobj.UID = child.value;
                    }
                });
            }
        }
