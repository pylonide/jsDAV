...
var jsCalDAV_Plugin = module.exports = jsDAV_Plugin.extend({
    ...
    unknownMethod: function(event, method, uri) {
         ...
         function afterCandidates(candidateNodes) {
             ...
             .end(function(err) {
                 ...
                 .end(function(err) {
                     ...
                     var prefer   = self.handler.getHTTPPrefer(),
                         response = self.handler.httpResponse;

                     if (response.headersSent) {
                         console.log("Error: headers already sent : ", response.statusCode);
                         response.end();
                     } else {
                         ...
                         response.end(self.handler.generateMultiStatus(result, prefer["return-minimal"]));
                     }
                 });
             });
         }
